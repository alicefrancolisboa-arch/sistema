...
function stockPage(){
 ...
 return head('CONTROLE DIÁRIO','Copos levados e complementos','Registre cada dia separadamente. O histórico permanece salvo mesmo quando você muda para outra data.')+'<section class="panel daily-cups"><label class="field"><strong>Data do controle</strong><input id="stock-day" type="date" value="'+stockDay+'"></label><form id="stock-form"><label class="field"><strong>Quantos copos você levou?</strong><input name="cups" type="number" min="0" max="100000" step="1" required value="'+(record?.cups??'')+'" placeholder="Ex.: 6"><small>Salvar novamente na mesma data apenas atualiza aquele dia.</small></label><button class="button" type="submit">Salvar quantidade de copos</button><p class="error-text" role="alert"></p></form></section><section class="stats"><div class="stat"><label>Copos levados</label><strong>'+(record?.cups??'—')+'</strong></div><div class="stat"><label>Vendidos no dia</label><strong>'+sold+'</strong></div><div class="stat"><label>Disponíveis para venda</label><strong>'+(record?Math.max(0,available):'—')+'</strong></div></section>'+(record&&available<0?'<div class="notice warn">Há '+(-available)+' vendas a mais que os copos informados. Confira o total levado.</div>':'')+'<section class="panel"><div class="panel-head"><h2>Histórico da semana</h2></div><p class="tiny muted">Cada linha mostra o que foi levado naquele dia, independentemente de ter vendido tudo.</p>'+(history||'<p class="tiny muted">Nenhum dia registrado ainda.</p>')+'</section><section class="panel"><div class="panel-head"><h2>Complementos no trabalho</h2>'+btn('＋ Complemento','complement','','secondary small')+'</div><p class="tiny muted">Informe a quantidade que ainda está no local.</p>'+(state.complements?.length?state.complements.map(c=>'<div class="schedule"><div class="grow"><strong>'+esc(c.name)+'</strong><p>'+c.quantity.toLocaleString('pt-BR')+' '+esc(c.unit)+'</p></div>'+btn('Atualizar','complement','data-id="'+c.id+'"','secondary small')+'</div>').join(''):empty('Cadastre os complementos','Ex.: leite em pó, granola ou creme.'))+'</section>';
}
function debtors(){
 const groups=new Map();for(const row of state.balances){const c=customer(row.customer);if(!c)continue;const item=groups.get(row.customer)||{customer:c, cents:0, qty:0, dues:new Map()};item.cents+=row.cents;item.qty+=row.qty;item.dues.set(row.due,(item.dues.get(row.due)||0)+row.cents);groups.set(row.customer,item);}
