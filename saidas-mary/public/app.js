...
function render(){
 if(!state)return;view=location.hash.slice(1)||'inicio';if(!['inicio','devendo','clientes','folhas','cobrancas','historico','estoque','ajustes'].includes(view))view='inicio';
 document.querySelectorAll('[data-nav]').forEach(a=>a.classList.toggle('active',a.dataset.nav===view));
 $('#main').innerHTML=({inicio:home,devendo:debtors,clientes:clients,folhas:sheets,cobrancas:billing,historico:history,estoque:stockPage,ajustes:settings})[view]();bindForms();
}
function home(){
 ...
 '<section class="quick-actions" aria-label="Ações rápidas"><a class="quick-action" href="#devendo"><strong>▣ Quem está devendo</strong><span>Veja em uma lista só os clientes com pagamento pendente.</span></a><a class="quick-action" href="#folhas"><strong>▤ Enviar foto da folha</strong><span>Escolha uma imagem ou tire uma foto dos risquinhos.</span></a><button class="quick-action" type="button" data-action="sale"><strong>＋ Venda manual</strong><span>Informe o cliente e a quantidade, sem precisar de foto.</span></button><a class="quick-action" href="#estoque"><strong>▦ Copos levados hoje</strong><span>Informe o total de copos, veja vendidos e o saldo disponível.</span></a></section>'+
...
function sheets(){
 ...
 '<section class="panel"><div class="panel-head"><h2>1. Escolha a remessa</h2>'+btn('＋ Nova folha','sheet','','secondary small')+'</div><div class="form-grid"><label class="field">Folha de anotações<select id="sheet-select"><option value="">Selecione uma folha…</option>'+options+'</select></label><label class="field">Data das compras acrescentadas<input id="photo-date" type="date" max="'+state.today+'" value="'+photoDate+'"><small>O sistema seleciona a remessa correspondente ao vencimento. Do dia 20 em diante, use a remessa do quinto dia útil.</small></label></div><p class="tiny muted">Cada remessa tem sua própria contagem. Quem já pagou na remessa anterior começa em zero na nova; quem ainda deve continua com a dívida antiga no financeiro, sem bloquear novas compras.</p></section>'+
 ...
 (photo?'<img class="preview" src="'+photo+'" alt="Foto da folha selecionada">'+btn('Ler nomes e risquinhos','read-photo',state.visionReady?'':'disabled','full')+'<p class="tiny muted">A foto será lida pelo Gemini. Se ele falhar, o OCR local tentará recuperar os nomes; confira e complete os totais dos risquinhos. A foto não fica armazenada.</p>':'')+'<p id="photo-error" class="error-text" role="alert"></p></section>'+(review?reviewHtml():'');
}
function reviewHtml(){
 ...
 '<p class="tiny muted">Leitor utilizado: <strong>'+(review.source==='ocr'?'OCR local em português':'Gemini'+(review.model?' · '+esc(review.model):''))+'</strong></p>'+(review.warning?'<div class="notice warn">'+esc(review.warning)+'</div>':'')+(review.rawText?'<details><summary>Ver texto recuperado pelo OCR</summary><pre class="ocr-text">'+esc(review.rawText)+'</pre></details>':'')+
 '<p class="tiny muted">Cada lado vale 1 açaí: quadrado fechado = 4 (R$ 40); quadrado com risco atravessando = 5 (R$ 50). A leitura conta o total acumulado visível; para um cliente existente, o sistema desconta automaticamente o total já confirmado e acrescenta só os novos. Confira cada linha com a foto.</p>'+
 ...
 (r.matchedCustomer?'<div class="notice">Já existe um cliente chamado <strong>'+esc(customer(r.matchedCustomer)?.name)+'</strong>. É a mesma pessoa?</div>':'')+'<label class="field">Quem comprou?<select required name="customer-'+i+'" data-match="'+i+'"><option value="">Escolha se é o mesmo cliente…</option><option value="__new__" '+(r.createNew||(!r.matchedCustomer&&!r.customer)?'selected':'')+'>Não, criar outro cliente com este nome</option>'+activeCustomers().map(c=>'<option value="'+c.id+'" '+(c.id===(r.customer||r.matchedCustomer)?'selected':'')+'>'+esc(c.name)+'</option>').join('')+'</select></label>'+
 '<label class="field">Data da compra deste cliente<input type="date" name="purchased-'+i+'" max="'+state.today+'" value="'+(r.purchased||photoDate)+'" required></label>'+'<label class="field">Total acumulado de risquinhos na foto<input name="total-'+i+'" data-total="'+i+'" type="number" min="0" max="10000" step="1" inputmode="numeric" required value="'+(r.total??'')+'"></label>'+
 (r.uncertain?'<span class="tag warn">Confira esta leitura com atenção</span>':'')+'<p class="tiny muted" data-note="'+i+'" '+(r.note?'':'hidden')+'>'+esc(r.note||'')+'</p>'+
 '<div class="counts"><span>Já contabilizados: <b data-previous="'+i+'">'+r.previous+'</b></span><strong data-delta="'+i+'">'+(r.total===null?'Novos: ?':r.total<r.previous?'Revisar: leitura menor que o acumulado':'Novos: '+(r.total-r.previous)+' · '+money((r.total-r.previous)*1000))+'</strong></div></div>').join('')+
...
function billing(){
 ...
 const dues=[...new Set(state.balances.filter(s=>s.due<=selectedDue).map(s=>s.due))].sort();
 const groups=dues.map(due=>({due,overdue:due<state.today,customers:state.customers.map(c=>({...c,balance:balanceForDue(c.id,due)})).filter(c=>c.balance>0)}));
 const overdueTotal=state.balances.filter(s=>s.due<state.today&&s.due<=selectedDue).reduce((a,s)=>a+s.cents,0),upcomingTotal=state.balances.filter(s=>s.due>=state.today&&s.due<=selectedDue).reduce((a,s)=>a+s.cents,0);
 return head('COBRANÇAS','Tudo pronto para lembrar seus clientes.','Confira os valores e envie as mensagens pelo WhatsApp.')+
 '<div class="toolbar"><select id="due-select" aria-label="Data limite da cobrança">'+dates().map(d=>'<option value="'+d+'" '+(d===selectedDue?'selected':'')+'>'+dueLabel(d)+'</option>').join('')+'</select>'+btn('Imprimir','print','','secondary')+'</div>'+
 '<div class="notice"><strong>Em atraso: '+money(overdueTotal)+' · A vencer: '+money(upcomingTotal)+'</strong><br>Valores separados pelo vencimento até '+prettyDate(selectedDue)+'. Compras com vencimento posterior não entram.</div>'+
 ...
 (groups.length?groups.map(g=>'<section class="panel billing-group"><div class="panel-head"><h2><span class="tag '+(g.overdue?'warn':'green')+'">'+(g.overdue?'Vencido':'A vencer')+'</span> '+prettyDate(g.due)+'</h2>'+btn('Copiar lista deste vencimento','copy-list','data-due="'+g.due+'"','secondary small')+'</div>'+g.customers.map(c=>{const message=billMessage(c,c.balance,g.due,g.overdue);return '<article class="billing-card"><div class="row spread"><h3>'+esc(c.name)+'</h3><span class="total">'+money(c.balance)+'</span></div><div class="message">'+esc(message)+'</div><div class="row">'+btn('Copiar mensagem','copy-message','data-id="'+c.id+'" data-due="'+g.due+'"','secondary small')+(c.phone?'<a class="button green small" target="_blank" rel="noopener noreferrer" href="https://wa.me/'+c.phone+'?text='+encodeURIComponent(message)+'">Abrir WhatsApp ↗</a>':btn('Cadastrar WhatsApp','customer','data-id="'+c.id+'"','secondary small'))+btn('Registrar pagamento','payment','data-id="'+c.id+'" data-cents="'+c.balance+'"','ghost small')+'</div></article>';}).join('')+'</section>').join(''):empty('Tudo em dia neste vencimento','Nenhum cliente tem saldo pendente até a data selecionada.'));
}
function balanceForDue(cid,due){return state.balances.filter(s=>s.customer===cid&&s.due===due).reduce((sum,s)=>sum+s.cents,0);}
function billMessage(c,cents,due,isOverdue){
 return isOverdue?'Olá, '+c.name+'! Seu pagamento de açaí de '+money(cents)+' venceu em '+prettyDate(due)+'. Quando puder, por favor, regularize. Obrigado! — Açaí da Mary':'Olá, '+c.name+'! Suas compras de açaí com vencimento em '+prettyDate(due)+' totalizam '+money(cents)+'. Por favor, faça o pagamento até essa data. Obrigado! — Açaí da Mary';
}
function history(){
...
function settings(){
 ...
 '<section class="panel"><h2>Usar no celular</h2><p class="tiny muted">O aplicativo Açaí da Mary usa as mesmas telas e dados do computador. Acesse saidas-mary.onrender.com ou instale o app. Funciona pela internet, com o computador desligado.</p>'+'<a class="button secondary full" href="/baixar-apk" download>↓ Baixar app para Android</a>'+btn('Ver como começar','instructions','','ghost small')+'</section></div></div>';
}
...
function captureReview(){
 ...
 review.rows.forEach((r,i)=>{r.name=f.get('name-'+i);const choice=f.get('customer-'+i);r.createNew=choice==='__new__';r.matchConfirmed=!!choice&&!r.createNew;r.customer=r.createNew?'':choice;r.purchased=f.get('purchased-'+i);r.total=f.get('total-'+i)===''?null:Number(f.get('total-'+i));r.previous=state.totals.find(t=>t.sheet===selectedSheet&&t.customer===r.customer)?.qty||0;if(r.total!==null&&r.total<r.previous){r.uncertain=true;r.note='A leitura ficou abaixo do acumulado já confirmado. Nenhuma quantidade ou dívida será reduzida. Confira a foto e corrija o total lido para continuar.';const input=$('[data-total="'+i+'"]');if(input)input.setCustomValidity('A leitura ficou abaixo do total já confirmado para este cliente. Revise o número com a foto.');}else{const input=$('[data-total="'+i+'"]');if(input)input.setCustomValidity('');if(r.countProtected){r.countProtected=false;r.uncertain=!!r.originalUncertain;r.note=r.originalNote||'';}}});
}
...
function bindForms(){
 ...
 const pd=$('#photo-date');if(pd)pd.onchange=()=>{photoDate=pd.value;review=null;const due=dueDate(photoDate,state.extraHolidays),match=state.sheets.filter(s=>s.due===due).sort((a,b)=>b.created.localeCompare(a.created))[0];if(match){selectedSheet=match.id;toast('Remessa de '+prettyDate(match.due)+' selecionada. A contagem desta remessa começa separada.');}else{selectedSheet='';toast('Esta é uma nova remessa. Crie uma nova folha para começar a contagem do zero.');}render();};
 ...
 if(rf){
  rf.oninput=()=>{captureReview();review.rows.forEach((r,i)=>{$('[data-previous="'+i+'"]').textContent=r.previous;const delta=r.total===null?null:r.total-r.previous;$('[data-delta="'+i+'"]').textContent=delta===null?'Novos: ?':delta<0?'Revisar: leitura menor que o acumulado':'Novos: '+delta+' · '+money(delta*1000);const note=$('[data-note="'+i+'"]');note.textContent=r.note||'';note.hidden=!r.note;});};rf.onchange=rf.oninput;
...
document.addEventListener('click',async e=>{
 ...
 try{
  ...
  if(action==='payment')paymentModal(cid,Number(button.dataset.cents)||undefined);
  ...
  if(action==='copy-message'){const due=button.dataset.due,c=customer(cid),cents=balanceForDue(cid,due);if(c&&cents)await copy(billMessage(c,cents,due,due<state.today));}
  if(action==='copy-list'){const due=button.dataset.due,list=state.customers.map(c=>({...c,balance:balanceForDue(c.id,due)})).filter(c=>c.balance>0);await copy((due<state.today?'Cobrança vencida':'Cobrança a vencer')+' em '+prettyDate(due)+'\n\n'+list.map(c=>c.name+' — '+money(c.balance)).join('\n')+'\n\nTotal: '+money(list.reduce((sum,c)=>sum+c.balance,0)));}
...
function weekStart(day){const d=new Date(day+'T12:00:00Z'),weekday=d.getUTCDay()||7;d.setUTCDate(d.getUTCDate()-weekday+1);return d.toISOString().slice(0,10);}
function weekLabel(start){const d=new Date(start+'T12:00:00Z'),end=new Date(d);end.setUTCDate(end.getUTCDate()+4);return prettyDate(start)+' a '+prettyDate(end.toISOString().slice(0,10));}
function stockPage(){
 ...
 const weeks=new Map();for(const r of state.dailyStock||[]){const key=weekStart(r.day),item=weeks.get(key)||{cups:0,sold:0};item.cups+=r.cups;item.sold+=state.sales.filter(s=>s.purchased===r.day).reduce((n,s)=>n+s.qty,0);weeks.set(key,item);}const weekly=[...weeks].sort((a,b)=>b[0].localeCompare(a[0])),currentWeek=weekStart(stockDay),current=weeks.get(currentWeek)||{cups:0,sold:0};
 const history=(state.dailyStock||[]).map(r=>{const daySold=state.sales.filter(s=>s.purchased===r.day).reduce((n,s)=>n+s.qty,0),left=r.cups-daySold;return '<div class="schedule"><div class="grow"><strong>'+prettyDate(r.day)+'</strong><p>'+r.cups+' copo(s) levados · '+daySold+' vendido(s)</p></div><strong class="right">'+Math.max(0,left)+' disponível(is)</strong></div>';}).join('');
 return head('CONTROLE DIÁRIO','Copos levados e complementos','Registre cada dia separadamente. O total semanal reinicia na segunda-feira e as semanas anteriores permanecem salvas.')+'<section class="panel daily-cups"><label class="field"><strong>Data do controle</strong><input id="stock-day" type="date" value="'+stockDay+'"></label><form id="stock-form"><label class="field"><strong>Quantos copos você levou?</strong><input name="cups" type="number" min="0" max="100000" step="1" required value="'+(record?.cups??'')+'" placeholder="Ex.: 6"><small>Salvar novamente na mesma data apenas atualiza aquele dia.</small></label><button class="button" type="submit">Salvar quantidade de copos</button><p class="error-text" role="alert"></p></form></section><section class="stats"><div class="stat"><label>Semana atual</label><strong>'+current.cups+' copo(s)</strong><small>'+weekLabel(currentWeek)+'</small></div><div class="stat"><label>Vendidos na semana</label><strong>'+current.sold+'</strong></div><div class="stat"><label>Disponíveis na semana</label><strong>'+Math.max(0,current.cups-current.sold)+'</strong></div></section>'+(record&&available<0?'<div class="notice warn">Há '+(-available)+' vendas a mais que os copos informados. Confira o total levado.</div>':'')+'<section class="panel"><div class="panel-head"><h2>Total por semana</h2></div><p class="tiny muted">A semana vai de segunda a sexta. Ao começar outra semana, o total atual começa novamente em zero.</p>'+(weekly.length?weekly.map(([start,item],i)=>'<div class="schedule"><div class="grow"><strong>Semana '+(weekly.length-i)+' · '+weekLabel(start)+'</strong><p>'+item.cups+' copo(s) levados · '+item.sold+' vendido(s)</p></div><strong class="right">'+Math.max(0,item.cups-item.sold)+' disponível(is)</strong></div>').join(''):'<p class="tiny muted">Nenhuma semana registrada ainda.</p>')+'</section><section class="panel"><div class="panel-head"><h2>Histórico por dia</h2></div><p class="tiny muted">Cada linha mostra o que foi levado naquele dia, independentemente de ter vendido tudo.</p>'+(history||'<p class="tiny muted">Nenhum dia registrado ainda.</p>')+'</section><section class="panel"><div class="panel-head"><h2>Complementos no trabalho</h2>'+btn('＋ Complemento','complement','','secondary small')+'</div><p class="tiny muted">Informe a quantidade que ainda está no local.</p>'+(state.complements?.length?state.complements.map(c=>'<div class="schedule"><div class="grow"><strong>'+esc(c.name)+'</strong><p>'+c.quantity.toLocaleString('pt-BR')+' '+esc(c.unit)+'</p></div>'+btn('Atualizar','complement','data-id="'+c.id+'"','secondary small')+'</div>').join(''):empty('Cadastre os complementos','Ex.: leite em pó, granola ou creme.'))+'</section>';
}
function debtors(){
 const groups=new Map();for(const row of state.balances.filter(row=>Number(row.cents)>0)){const c=customer(row.customer);if(!c)continue;const item=groups.get(row.customer)||{customer:c, cents:0, qty:0, dues:new Map()};item.cents+=row.cents;item.qty+=row.qty;item.dues.set(row.due,(item.dues.get(row.due)||0)+row.cents);groups.set(row.customer,item);}
 const list=[...groups.values()].sort((a,b)=>b.cents-a.cents);
 return head('COBRANÇAS PENDENTES','Quem está devendo','Clientes que ainda têm saldo em aberto, com o valor e o vencimento.', '<a class="button secondary" href="#inicio">← Voltar</a>')+
 '<section class="panel"><div class="panel-head"><h2>'+list.length+' cliente(s) com pagamento pendente</h2><strong>'+money(list.reduce((n,x)=>n+x.cents,0))+'</strong></div>'+(list.length?list.map(item=>'<article class="schedule"><div class="grow"><strong>'+esc(item.customer.name)+'</strong><p>'+item.qty+' açaí(s) em aberto · '+[...item.dues].sort((a,b)=>a[0].localeCompare(b[0])).map(([due,cents])=>prettyDate(due)+' · '+money(cents)).join(' · ')+'</p></div><strong class="right">'+money(item.cents)+'</strong>'+btn('Receber','payment','data-id="'+item.customer.id+'"','secondary small')+'</article>').join(''):'<div class="empty"><strong>Ninguém está devendo</strong><p>Todos os pagamentos estão em dia.</p></div>')+'</section>';
}
