...
function captureReview(){
 ...
 review.rows.forEach((r,i)=>{r.name=f.get('name-'+i);const choice=f.get('customer-'+i);r.createNew=choice==='__new__';r.matchConfirmed=!!choice&&!r.createNew;r.customer=r.createNew?'':choice;r.purchased=f.get('purchased-'+i);r.total=f.get('total-'+i)===''?null:Number(f.get('total-'+i));r.previous=state.totals.find(t=>t.sheet===selectedSheet&&t.customer===r.customer)?.qty||0;if(r.total!==null&&r.total<r.previous){if(!r.countProtected){r.originalUncertain=r.uncertain;r.originalNote=r.note;}r.countProtected=true;r.total=r.previous;r.uncertain=true;r.note='A leitura ficou abaixo do que já foi registrado. O aplicativo manteve o total anterior para não subtrair vendas. Confira a folha e informe o total acumulado correto.';const input=$('[data-total="'+i+'"]');if(input)input.value=String(r.previous);}else if(r.countProtected&&r.total>r.previous){r.countProtected=false;r.uncertain=!!r.originalUncertain;r.note=r.originalNote||'';}});
}
...
function bindForms(){
 ...
 if(rf){
  rf.oninput=()=>{captureReview();review.rows.forEach((r,i)=>{$('[data-previous="'+i+'"]').textContent=r.previous;$('[data-delta="'+i+'"]').textContent=r.total===null?'Novos: ?':'Novos: '+(r.total-r.previous)+' · '+money((r.total-r.previous)*1000);const note=$('[data-note="'+i+'"]');note.textContent=r.note||'';note.hidden=!r.note;});};rf.onchange=rf.oninput;
