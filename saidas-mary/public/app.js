...
function reviewHtml(){
 ...
 '<p class="tiny muted">Cada lado vale 1 açaí: quadrado fechado = 4 (R$ 40); quadrado com risco atravessando = 5 (R$ 50). “Total acumulado” inclui os risquinhos já contabilizados. Se a leitura não contou riscos novos, corrija no campo ao lado: somente a diferença será adicionada. Confira cada linha com a foto.</p>'+
 ...
 '<label class="field">Data da compra deste cliente<input type="date" name="purchased-'+i+'" max="'+state.today+'" value="'+(r.purchased||photoDate)+'" required></label>'+'<label class="field">Total acumulado de risquinhos<input name="total-'+i+'" data-total="'+i+'" type="number" min="0" max="10000" step="1" inputmode="numeric" required value="'+(r.total??'')+'"></label>'+'<label class="field">Risquinhos novos que a leitura não contou<input name="extra-'+i+'" type="number" min="0" max="10000" step="1" inputmode="numeric" value="'+(r.extra||0)+'"><small class="muted">Exemplo: a leitura mostra 8, mas você vê 3 risquinhos novos? Informe 3.</small></label>'+
 ...
 '<div class="counts"><span>Já contabilizados: <b data-previous="'+i+'">'+r.previous+'</b></span><strong data-delta="'+i+'">Novos: '+(r.total===null?'?':Math.max(r.total,r.previous)-r.previous+(r.extra||0))+'</strong></div></div>').join('')+
...
function captureReview(){
 ...
 review.rows.forEach((r,i)=>{r.name=f.get('name-'+i);const choice=f.get('customer-'+i);r.createNew=choice==='__new__';r.matchConfirmed=!!choice&&!r.createNew;r.customer=r.createNew?'':choice;r.purchased=f.get('purchased-'+i);r.total=f.get('total-'+i)===''?null:Number(f.get('total-'+i));r.extra=Number(f.get('extra-'+i)||0);r.previous=state.totals.find(t=>t.sheet===selectedSheet&&t.customer===r.customer)?.qty||0;if(r.total!==null&&r.total<r.previous){if(!r.countProtected){r.originalUncertain=r.uncertain;r.originalNote=r.note;}r.countProtected=true;r.total=r.previous;r.uncertain=true;r.note='A leitura ficou abaixo do que já foi registrado. O total anterior foi mantido. Informe os riscos novos que faltaram no campo de correção.';const input=$('[data-total="'+i+'"]');if(input)input.value=String(r.previous);}else if(r.countProtected&&r.total>r.previous){r.countProtected=false;r.uncertain=!!r.originalUncertain;r.note=r.originalNote||'';}});
}
