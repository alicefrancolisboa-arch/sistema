...
export function createApp({dbPath=join(process.env.DATA_DIR||join(root,'data'),'acai.sqlite'),apiKey=process.env.GEMINI_API_KEY||'',password=process.env.APP_PASSWORD||'',model=process.env.GEMINI_MODEL||'gemini-3.6-flash',vision=recognize}={}){
 if(dbPath!==':memory:')mkdirSync(dirname(dbPath),{recursive:true});
 const store=new Store(dbPath),sessions=new Map(),drafts=new Map(),attempts=new Map();
 let reading=false;
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  try{
   ...
   if(path.startsWith('/api/')){
    ...
    if(path==='/api/state'&&req.method==='GET')return send(res,200,{...store.snapshot(),visionReady:true,geminiConfigured:!!apiKey,ocrReady:true,protected:!!password,model});
    if(path==='/api/backup'&&req.method==='GET')return send(res,200,store.backup(),{'Content-Disposition':'attachment; filename="acai-backup-'+new Date().toISOString().slice(0,10)+'.json"'});
    if(req.method!=='POST')return send(res,404,{error:'Página não encontrada.'});
    const b=await body(req);
    if(path==='/api/customers')return send(res,200,store.saveCustomer(b));
    if(path==='/api/sales')return send(res,200,store.sale(b));
    if(path==='/api/payments')return send(res,200,store.payment(b));
    if(path==='/api/payments/undo')return send(res,200,store.undoPayment(b.id));
    if(path==='/api/sheets')return send(res,200,store.createSheet(b));
    if(path==='/api/holidays')return send(res,200,store.setExtras(b.dates));
    ...
    if(path==='/api/restore'){
     ...
     if(dbPath!==':memory:')writeFileSync(join(dirname(dbPath),'antes-restauracao-'+Date.now()+'.json'),JSON.stringify(store.backup()));
     const result=store.restore(b.backup);drafts.clear();return send(res,200,result);
    }
    if(path==='/api/photos/read'){
     const sheet=store.sheet(b.sheet);
     ...
     if(store.imported(hash))throw Error('Esta foto já foi contabilizada. Envie uma foto atualizada da mesma folha.');
     ...
     const customers=store.snapshot().customers,totals=store.totals(sheet.id),id=randomUUID();
    ...
    if(path==='/api/photos/confirm'){
     ...
     const result=store.commit({...b,sheet:draft.sheet,hash:draft.hash});return send(res,200,result);
    }
  ...
  }catch(e){send(res,400,{error:e.name==='TimeoutError'?'A leitura demorou demais. Tente uma foto mais nítida.':e.message.includes('SQLITE')?'Não foi possível salvar. Confira os dados.':e.message});}
 });
...
if(process.argv[1] && fileURLToPath(import.meta.url)===process.argv[1]){
 ...
 if(process.env.RENDER&&!process.env.DATA_DIR)throw Error('Configure DATA_DIR com o caminho do disco persistente do Saídas Mary.');
 const {server}=createApp();server.listen(port,host,()=>console.log('Saídas Mary disponível em http://'+host+':'+port+' — leitura de fotos: '+(process.env.GEMINI_API_KEY?'Gemini configurado + OCR local':'OCR local; Gemini ainda não configurado')));
}
