...
savePurchase = async function(e){e.preventDefault();const form=e.target;if(form.dataset.saving==='1')return;form.dataset.saving='1';const submit=form.querySelector('button.primary');submit.disabled=true;try{let lines=typeof purchasePayload==='function'?purchasePayload():[...purchaseItems.children].map(x=>({ingredient:x.children[0].value,quantity:+x.children[1].value,unit_cost:+x.children[2].value}));await api(editingPurchase?'/api/purchases/'+editingPurchase:'/api/purchases',{method:editingPurchase?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({supplier:supplier.value,items:lines})});editingPurchase=null;closeModal('purchaseModal');toast('Compra salva e custos atualizados!');await stock();purchaseHistory();dashboard()}catch(error){toast(error.message)}finally{form.dataset.saving='';submit.disabled=false}}
