...
async function editIngredient(id){
  ...
  iName.value=x.name;iCategory.value=x.category||'insumo';iUnit.value=x.unit;iStock.value=x.stock;iCost.value=x.cost;iPackageSize.value=x.package_size||'';iPackageTotal.value=x.package_size>0?(Number(x.cost)*Number(x.package_size)).toFixed(2):'';togglePackageSize();recalculatePackageCost(); document.querySelector('#ingredientModal h2').textContent='Editar item de estoque';document.querySelector('#ingredientModal').classList.add('show');
}
